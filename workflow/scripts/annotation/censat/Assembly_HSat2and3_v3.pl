...
my $usage = "perl Assembly_HSat2and3_v3.pl /path/to/reference.fasta /path/to/output_dir";
...
if(defined $ARGV[1]){
	...
	die "no output_dir specified!\n\nusage: $usage\n\n";
}

#create output file name
my $outfile = "";
if($reffile=~/.+\/(.+)\..+?$/){
	$outfile = "$output_dir/HSat2and3_Regions.bed";
}
else{
...
my $tempfileA = $output_dir . "/tempfileA.bed";
open(OUTA,'>', $tempfileA);
...
open(IN, $tempfileA);
my $tempfileC = $output_dir . "/tempfileC.bed";
open(OUTC,'>', $tempfileC);
...
my $tempfileD = $output_dir . "/tempfileD.bed";
open(OUTD,'>', $tempfileD);
if(1==1){#use true conditional to restrict scope of contained variables
