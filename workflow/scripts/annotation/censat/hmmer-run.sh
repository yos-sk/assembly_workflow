...
#   FEDOR: addicional overlap filtering
    python3 /opt/HumAS-HMMER_for_AnVIL/overlap_filter.py ${dir}/_nhmmer-t0-$bn.bed > ${dir}/AS-HOR+SF-vs-$bn.bed

#   FEDOR: AS-HOR only (skip SF monomers)
    awk '{ if (length($4)==2) {next} print}' ${dir}/AS-HOR+SF-vs-$bn.bed > ${dir}/AS-HOR-vs-$bn.bed
